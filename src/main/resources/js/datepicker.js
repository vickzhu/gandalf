...
window.calender = (function(win,doc){
	...
	var m_en = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
	C.prototype = {
		init : function(){
			var t = this;
		...
		createMon : function(){
			...
			m = m == 0 ? 12 : m;
			for(var i = 1;i<=12;i++){
				html+='<div class="calender-mon-cell '+( m == i ? 'active' : '') +' " mid="'+i+'">'+ m_en[i-1] +'</div>';
			};
			...
			for(var i2 = 0;i2<cells.length;i2++){
			 	cells[i2].onclick = function(){
			 		...
			 		t.del(g('.calender-wrap'),'month');
			 		t.nt.setDate(1);
					t.nt.setMonth(this.getAttribute('mid')-1);
					t.createDay();
			 	}
			}
		},
		...
		createDay : function(n){
			...
			var m_show = m_en[m === 0 ? 11 : m-1];
			g('#calender-mon').innerHTML = m_show;
		...
		create : function(){
			var t= this;
			if( g('.calender-wrap')) doc.body.removeChild( g('.calender-wrap') )
			var private_Day_title=['Mo','Tu','We','Th','Fr','Sa','Su'];
			//内容
			var html = '<div class="calender-wrap">';
			html +='<div id="calender-header" class="calender-header none-btn "><a id="calender-prev" href="javascript:;">&lt;</a><a id="calender-next" href="javascript:;">&gt;</a> <span id="calender-mon">10</span><span id="calender-year">2016</span></div>'
			//星期
